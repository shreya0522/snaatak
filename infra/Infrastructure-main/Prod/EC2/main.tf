...
module "redis02" {
  source                      = "git@github.com:OT-MyGurukulam/EC2_tf_module.git?ref=main"
  security_groups             = try(toset([data.terraform_remote_state.security_config.outputs.redis_sg02_id]), null)
  key_name                    = var.key_name
  subnet_id                   = try(data.terraform_remote_state.network_config.outputs.db02_private_subnet_id, null)
  instance_profile_name       = var.instance_profile_name
  name                        = var.redis01.name
  ami                         = var.redis01.ami
  tool                        = var.redis01.tool
...
module "scylla01" {
  ...
  //ebs_size                    = var.scylla_ebs_size
}
module "scylla02" {
  source                      = "git@github.com:OT-MyGurukulam/EC2_tf_module.git?ref=main"
  security_groups             = try(toset([data.terraform_remote_state.security_config.outputs.scylla_sg02_id]), null)
  key_name                    = var.key_name
  subnet_id                   = try(data.terraform_remote_state.network_config.outputs.db02_private_subnet_id, null)
  instance_profile_name       = var.instance_profile_name
  name                        = var.scylla01.name
...
module "postgress02" {
  source                      = "git@github.com:OT-MyGurukulam/EC2_tf_module.git?ref=main"
  security_groups             = try(toset([data.terraform_remote_state.security_config.outputs.postgres_sg02_id]), null)
  key_name                    = var.key_name
  subnet_id                   = try(data.terraform_remote_state.network_config.outputs.db02_private_subnet_id, null)
  instance_profile_name       = var.instance_profile_name
  name                        = var.postgress01.name
